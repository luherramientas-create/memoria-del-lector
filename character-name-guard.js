...
(function(){
  ...
  function warning(title,intro,items,buttons){
    modal(`<h2>${title}</h2><p>${intro}</p><div class="list">${items.map(c=>`<div class="card"><h3>${esc(c.name)}</h3>${stats(active(),c)}<div class="actions"><button type="button" class="secondary" onclick="editCharacter('${c.id}')">Revisar personaje</button></div></div>`).join('')}</div><div class="actions">${buttons}</div>`);
  }
  ...
  function createCharacter(e){
    ...
    if(m.exact.length){
      warning('⚠️ Ya existe un personaje con este nombre','Ya tienes registrado un personaje con este nombre. Podrían ser la misma persona, pero también pueden ser personajes diferentes.',m.exact,[`<button type="button" class="secondary" onclick="close()">Cancelar</button>`,`<button type="button" class="primary" id="createAnyway">Crear de todas formas</button>`]);
    }else{
      warning('🔎 Hay un personaje con un nombre similar','Podría tratarse de la misma persona. Revisa el registro antes de continuar.',m.similar,[`<button type="button" class="secondary" onclick="close()">Cancelar</button>`,`<button type="button" class="primary" id="createAnyway">Crear de todas formas</button>`]);
    }
  ...
  function updateCharacter(e,id){
    ...
    if(m.exact.length){
      warning('⚠️ Ya existe un personaje con este nombre','Otro personaje del mismo libro ya utiliza este nombre.',m.exact,[`<button type="button" class="secondary" onclick="close()">Cancelar</button>`,`<button type="button" class="primary" id="updateAnyway">Guardar de todas formas</button>`]);
    }else{
      warning('🔎 Hay un personaje con un nombre similar','El nombre editado se parece a otro personaje de este libro. Revisa antes de continuar.',m.similar,[`<button type="button" class="secondary" onclick="close()">Cancelar</button>`,`<button type="button" class="primary" id="updateAnyway">Guardar de todas formas</button>`]);
    }
